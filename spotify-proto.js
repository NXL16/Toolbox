...
try {
    ...
    if (url.includes("bootstrap/v1/bootstrap") && method === "POST") {
        const BootstrapResponse = root.lookupType("BootstrapResponse");
        let obj = BootstrapResponse.decode(binaryBody);

        const accountAttributes = obj?.ucsResponseV0?.success?.customization?.success?.accountAttributesSuccess?.accountAttributes;

        if (accountAttributes) {
            processMapObj(accountAttributes);
            if (obj.ucsResponseV0?.success?.customization?.success) {
                obj.ucsResponseV0.success.customization.success.fetchTimeMillis = protobuf.util.Long.fromNumber(Date.now());
            }
        }

        body = BootstrapResponse.encode(obj).finish();
        console.log('✅ Bootstrap patched');

    } else if (url.includes("user-customization-service/v1/customize") && method === "POST") {
        const UcsResponseWrapper = root.lookupType("UcsResponseWrapper");
        let obj = UcsResponseWrapper.decode(binaryBody);

        const accountAttributes = obj?.success?.accountAttributesSuccess?.accountAttributes;

        if (accountAttributes) {
            processMapObj(accountAttributes);
            if (obj.success?.accountAttributesSuccess) {
                obj.success.accountAttributesSuccess.fetchTimeMillis = protobuf.util.Long.fromNumber(Date.now());
            }
        }

        body = UcsResponseWrapper.encode(obj).finish();
        console.log('✅ Customize patched');
    } else {
        $done({});
        return;
    }
